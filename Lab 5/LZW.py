...
def lzw_compress(input_file, output_file, max_bits=12, 
                 reset_dict=True, use_bwt=False, use_mtf=True):
    ...
    # Збереження параметрів у заголовку
    header = bytes([max_bits, reset_dict, use_bwt, use_mtf])
...
def lzw_decompress(input_file, output_file):
    with open(input_file, 'rb') as f_in, open(output_file, 'wb') as f_out:
        ...
        use_bwt = bool(header[2])
        use_mtf = bool(header[3])
    ...
    #     if use_mtf:
    #         # data = []
    #         for i in range(0, len(data_out), 256):
    #             f_out.write(imtf_transform(data_out[i:i+256]))
    #         # data_out = data
    ...
    #     if use_bwt:
    #         # data = []
    #         for i in range(0, len(data_out), 257):
    #             f_out.write(ibwt_transform(data_out[i+1:i+257], data_out[i]))
